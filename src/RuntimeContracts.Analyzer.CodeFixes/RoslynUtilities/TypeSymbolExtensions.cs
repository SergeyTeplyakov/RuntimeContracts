...
internal static partial class ITypeSymbolExtensions
{
        ...
        if (type != null)
        {
            switch (type.Kind)
            {
                case SymbolKind.NamedType:
                    ...
                    {
                        if (baseType.Equals(compilation.ExceptionType(), SymbolEqualityComparer.Default))
                        {
                            return true;
                        }
                    }
    ...
    public static bool IsDisposable([NotNullWhen(returnValue: true)] this ITypeSymbol? type, [NotNullWhen(returnValue: true)] ITypeSymbol? iDisposableType)
        => iDisposableType != null &&
           (iDisposableType.Equals(type, SymbolEqualityComparer.Default) ||
            type?.AllInterfaces.Contains(iDisposableType, SymbolEqualityComparer.Default) == true);
